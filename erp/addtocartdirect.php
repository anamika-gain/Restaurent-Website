...
foreach ($productSizeDetails as $key => $value) {
    ...
    if ($productSizeDetails[$key]['special_price_from'] < $currentDate && $productSizeDetails[$key]['special_price_to'] > $currentDate) {
        $productSizePrice = $productSizeDetails[$key]['special_price'];
    } else {
        $productSizePrice = $productSizeDetails[$key]['selling_price'];
    }

    $productSizeId = $productSizeDetails[$key]['id'];
}
...
if ($addNew == 1) {
    ...
    $newItem = array(
        ...
        "product_extra_option" => 0,
        "product_extra_option_price" => 0,
